...
function mapRow(row) {
  ...
  return {
    ...
    ...safeJsonParse(row.shared_agent_binding_json, {}),
...
export const yeaftSessionDb = {
  ...
  upsertFromSnapshot(userId, agentId, session) {
    ...
    stmts.upsertYeaftSession.run(
      ...
      safeJsonStringify({
        ...(typeof session.sharedAgentDefinitionId === 'string' ? { sharedAgentDefinitionId: session.sharedAgentDefinitionId } : {}),
        ...(Number.isInteger(session.sharedAgentDefinitionRevision) ? { sharedAgentDefinitionRevision: session.sharedAgentDefinitionRevision } : {}),
        ...(typeof session.sharedAgentDefinitionName === 'string' ? { sharedAgentDefinitionName: session.sharedAgentDefinitionName } : {}),
      }),
